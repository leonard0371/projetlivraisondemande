...
class MontrealDeliveryDataGenerator:
    ...
    def choisir_arrondissement(self):
        arrs = list(self.arrondissements_config)
        poids = [cfg['poids'] for cfg in self.arrondissements_config.values()]
        return np.random.choice(arrs, p=poids)

    def choisir_categorie_produit(self):
        cats = list(self.produits_config)
        w = [cfg['poids'] for cfg in self.produits_config.values()]
    ...
    def generer_commande(self, date_base, now):
        ...
        arr = self.choisir_arrondissement()
        cat = self.choisir_categorie_produit()
        ...
        prix_total = round(pu*qte*1.14975,2)
    ...
    def generer_dataset(self, nb_jours=150, nb_commandes_par_jour_range=(10,2000), prob_jour_sans_commande=0.02):
        ...
        Affiche pour chaque jour le nombre de commandes générées (affichage intempestif).
        """
        ...
        for j, n_cmd in enumerate(nb_par_jour):
            ...
            print(f"Jour {date_cour.date()} : {n_cmd} commandes générées")
